...
bot.on('message', async (msg) => {
  try {
    ...
    if (debugMode) console.log(`[INFO] Received message from ${username}: ${text}`);

    if (text.startsWith('/')) {
      if (debugMode) console.log(`[INFO] Handling command: ${text}`);
    ...
    if (text.startsWith('cashuA')) {
      try {
        ...
        if (debugMode) console.log(`[INFO] Detected Cashu token from ${username}`);
        await handleMessage(bot, msg, cashuApiUrl, claimedDisposeTiming);
      } catch (error) {
        if (debugMode) console.log(`[INFO] No valid Cashu token detected in the message from ${username}`);
        if (msg.chat.type === 'private') {
          if (debugMode) console.log(`[INFO] Sending help message to ${username}`);
          await bot.sendMessage(chatId, messages.helpMessage);
        }
      }
    } else if (msg.chat.type === 'private') {
      if (debugMode) console.log(`[INFO] No valid Cashu token and not a command. Sending help message to ${username}`);
      await bot.sendMessage(chatId, messages.helpMessage);
    }
  } catch (error) {
